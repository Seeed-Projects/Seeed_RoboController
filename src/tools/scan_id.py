...
try:
    ...
    print(tr("❌ 错误: 未找到 scservo_sdk。请确保 scservo_sdk 文件夹在当前目录下。"))
...
try:
    ...
    print(tr("❌ 错误: 未找到 port_utils。请确保 port_utils.py 在当前目录下。"))
...
def scan_servos(port_name: str) -> list:
    ...
    """
    print(f"\n{'='*50}")
    print(tr("📡 舵机扫描工具 / Servo ID Scanner"))
    print(f"{'='*50}")
    print(tr("端口 / Port: {}").format(port_name))
    print(tr("扫描范围 / Scan Range: ID 1 - {}").format(MAX_ID))
    print(f"{'='*50}\n")
    ...
    # 开始扫描
    print(tr("🔍 扫描中 / Scanning...\n"))
    print("-" * 50)
    print(f"{'ID':<5} | {tr('型号 (Model)'):<15} | {tr('状态')}")
    print("-" * 50)
    ...
    for servo_id in range(1, MAX_ID + 1):
        ...
        if result == COMM_SUCCESS:
            print(f"{servo_id:<5} | {model_number:<15} | {tr('✅ 在线 / Online')}")
    ...
    print("-" * 50)
    print(tr("\n📊 扫描结束 / Scan Complete"))
    print(tr("   发现 {} 个舵机 / Found {} servo(s): {}").format(len(found), len(found), found))
    print(f"{'='*50}\n")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        # 交互式选择端口
        port_name = select_port_interactive(tr("选择要扫描的串口 / Select port to scan"))
        if not port_name:
            print(tr("❌ 未选择端口，退出 / No port selected, exit"))
            sys.exit(1)
