...
def calibrate_joint(servo_handler, servo_id: int, joint_name: str) -> Dict:
    ...
    print(f"\n{'='*60}")
    print(f"🔧 校准关节 / Calibrating joint: ID{servo_id} - {display_name}")
    print(f"{'='*60}")

    # 失能力矩
    enable_torque(servo_handler, [servo_id], enable=False)
    print("⏹️ 力矩已关闭，请手动移动该关节")
    print("   Torque disabled, please move this joint manually\n")

    # 步骤 1：记录中位 / homing_offset
    input(f"请将 {display_name} 移动到其运动范围的中间位置，然后按回车...\n"
          f"Move {joint_name} to the MIDDLE of its range and press ENTER...")

    homing_offset = read_position(servo_handler, servo_id)
    if homing_offset is None:
        print(f"❌ 无法读取 ID{servo_id} 位置，使用默认值 2048")
        homing_offset = 2048
    else:
        print(f"✅ 中位已记录 / Homing offset recorded: {homing_offset}\n")

    # 步骤 2：记录运动范围
    if joint_name in CONTINUOUS_JOINTS:
        range_min = 0
        range_max = 4095
        print(f"🔄 {display_name} 为连续旋转关节，范围固定为 [0, 4095]\n")
    else:
        input(f"请缓慢将 {display_name} 在其完整运动范围内来回移动，然后按回车停止记录...\n"
              f"Slowly move {joint_name} through its FULL RANGE of motion, then press ENTER...")

        print("📡 正在记录范围 / Recording range...")
        range_min = 4095
        range_max = 0
        record_count = 0
        ...
        if record_count == 0:
            print("⚠️ 未记录到有效位置，使用默认范围 [0, 4095]")
            range_min = 0
            range_max = 4095
        else:
            print(f"✅ 范围已记录 / Range recorded: [{range_min}, {range_max}]\n")
...
def calibrate_arm(port_name: str, save_path: str, arm_type: str = "follower") -> bool:
    ...
    """
    print(f"\n{'='*70}")
    print(f"🦾 LeRobot 风格机械臂校准 / LeRobot-style Arm Calibration")
    print(f"{'='*70}")
    print(f"串口 / Port: {port_name}")
    print(f"臂类型 / Arm type: {arm_type}")
    ...
    if not calib_servos:
        print("❌ 未发现 SO-10x 标准关节舵机（ID 1-6）")
        port_handler.closePort()
        return False

    print(f"将校准以下舵机 / Will calibrate: {calib_servos}\n")

    # 先全部失能，方便手动调整
    enable_torque(servo_handler, calib_servos, enable=False)
    print("💡 所有待校准舵机已失能，可以手动移动\n")
    ...
    try:
        ...
        print(f"\n✅ 校准文件已保存 / Calibration saved to: {save_path}\n")
    except Exception as e:
        print(f"\n❌ 保存校准文件失败 / Failed to save calibration: {e}\n")
        port_handler.closePort()
        return False

    # 测试：移动到中位
    print("🎯 校准完成，是否移动到中位验证？")
    try:
        confirm = input("按 'y' 回车移动到中位验证，其他键跳过: ").strip().lower()
        if confirm == 'y':
            print("\n⚡ 启动力矩...")
            ...
            for servo_id in sorted(calib_servos):
                ...
                if pos is not None:
                    diff = pos - target
                    print(f"  ID{servo_id} ({JOINT_NAME_MAP.get(joint_name, joint_name)}): "
                          f"当前 {pos:4d} | 目标 {target:4d} | 偏差 {diff:+4d}")
            print("-" * 60)
...
标准操作流程:
  ...
  4. 保存 JSON 校准文件
    ...
    parser.add_argument('--arm-type', choices=['follower', 'leader'], default='follower',
                       help='臂类型（默认: follower）')
    parser.add_argument('--id', default=None, help='校准文件 ID 名称（默认自动生成）')
    parser.add_argument('--output', default=None, help='自定义输出文件路径')
    ...
    if args.port:
        ...
        port_name = select_port_interactive("选择要校准的串口 / Select port to calibrate")
        if not port_name:
    ...
    if args.output:
        ...
        if args.id:
            ...
            try:
                file_id = input(f"请输入校准文件 ID（直接回车使用 {default_name}）: ").strip()
            except (EOFError, KeyboardInterrupt):
                file_id = ""
            if not file_id:
